...
placeholders: ## Report template text still unfilled in the canonical files (STRICT=1 to fail on it)
	@hits=$$(grep -nEH '\[repo name\]|\[Repository Name\]|\[fill:|_\[[^]]+\]_|\[repo_kind\]|\[list\]|\[add as you grow|\[add yours as you hit them\]|\[the single canonical path\]|\[the ones set true\]|One-line description of what this repo does|\[Describe what this repository does|Replace me|^# Pack: |<upcoming milestone|<risk or blocker>|<one-line rationale>' $(PLACEHOLDER_PATHS) 2>/dev/null || true); \
